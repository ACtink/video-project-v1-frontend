    ...
    <>
      ...
      <div className="bg-[#0a0a0a] max-w-[470px] mx-auto">
          ...
          >
            {post.user.profilePicture ? (
              <img
                src={post.user.profilePicture}
                alt=""
                className="w-8 h-8 rounded-full object-cover border border-white/10 ring-2 ring-neutral-700 flex-shrink-0"
              />
            ) : (
              <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                {post.user.username?.charAt(0).toUpperCase()}
            ...
            {/* Username: xs on mobile → sm on sm+ screens */}
            <span className="font-semibold text-xs sm:text-sm text-neutral-100 tracking-wide truncate">
              {post.user.username}
            </span>
          </div>
        ...
        <div className="px-3.5 pt-2.5 pb-4">
          ...
          <p className="text-xs sm:text-sm font-semibold text-neutral-100 mb-1 tracking-wide">
            {likesCount.toLocaleString()} {likesCount === 1 ? "like" : "likes"}
          </p>

          {post.caption && (
            /* Caption body: xs on mobile → sm on sm+ */
            <p className="text-xs sm:text-sm leading-relaxed text-white/75 mb-1.5">
              <span className="font-semibold text-neutral-100 mr-1.5">
                {post.user.username}
              </span>
              {post.caption}
          ...
          {/* "View all comments" link: xs on mobile → sm on sm+ */}
          <p
            onClick={() => {
              if (window.innerWidth < 768) setShowMobileComments(true);
              else setSelectedPost(post);
            }}
            className="text-xs sm:text-sm text-white/35 cursor-pointer mb-2 tracking-wide"
          >
            View all {commentsCount} comments
          </p>

          {/* Date stamp: [10px] on mobile → xs on sm+ */}
          <p className="text-[10px] sm:text-xs text-white/25 mb-2.5 uppercase tracking-widest">
            {new Date(post.createdAt).toLocaleDateString("en-US", {
              month: "long",
              day: "numeric",
            })}
