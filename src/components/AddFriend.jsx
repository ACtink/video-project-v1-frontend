...
function AddFriend({ uiState, allGoodAndConnected }) {
  const { strangerUserProfileData } = useContext(webRTCContext);

  const [isFollowing, setIsFollowing] = useState(false);
  const [ripple, setRipple] = useState(false);

  const handleFollowUser = async () => {
    if (isFollowing || uiState === "idle") return; // ✅ fixed condition
    setRipple(true);
    setTimeout(() => setRipple(false), 600);
    try {
      const res = await fetchData(
        `/api/users/${strangerUserProfileData?.data?.id}/direct-follow`, // ✅ direct follow, no request needed
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    if (!strangerUserProfileData?.data?.id) return;
    setIsFollowing(false); // reset on new stranger
    const checkFollowStatus = async () => {
      try {
        const res = await fetchData(
          `/api/users/${strangerUserProfileData.data.id}/is-following`,
        ...
        const data = await res.json();
        setIsFollowing(data.followStatus === "following"); // ✅ fixed, was data.isFollowing which doesn't exist
      } catch (err) {
        console.error("Follow status error", err);
      }
    };
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={handleFollowUser}
        disabled={isFollowing || uiState === "idle"} // ✅ fixed condition
        aria-label={isFollowing ? "Already friends" : "Add as friend"}
      >
        {allGoodAndConnected && (
          <>
