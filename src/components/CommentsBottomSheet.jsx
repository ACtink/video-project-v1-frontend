...
// // ─── Avatar ───────────────────────────────────────────────────────────────────
// function Avatar({ username = "?", profilePicture, size = 32 }) {
//   const color = `hsl(${(username.charCodeAt(0) * 47) % 360}, 50%, 42%)`;
//   return (
//     <div
//       style={{
...
//             <div
//               className="sk-pulse"
//               style={{ width: "30%", height: 10, borderRadius: 5 }}
//             />
//             <div
//               className="sk-pulse"
//               style={{ width: `${w}%`, height: 10, borderRadius: 5 }}
//             />
//           </div>
//         </div>
//       ))}
...
//         No comments yet
//       </p>
//       <p style={{ margin: 0, fontSize: 12, color: "rgba(255,255,255,0.13)" }}>
...
//     if (dragStartScroll.current > 2) return; // only dismiss when list at top
//     const delta = e.touches[0].clientY - dragStartY.current;
//     if (delta > 0) setDragY(delta);
//   };
...
//       {/*
//        * Sheet uses position:fixed + bottom:0.
//        * The key trick for keyboard behavior: we do NOT set a fixed height.
//        * Instead we set max-height: calc(100dvh - env(safe-area-inset-top, 0px)).
//        * When the iOS/Android keyboard opens, it shrinks the visual viewport
//        * (dvh units), so the sheet's max-height naturally shrinks — no JS needed.
//        * The list (flex:1 + overflow-y:auto) absorbs the compression.
//        * The input bar (flex-shrink:0) is always pinned at bottom of sheet.
//        */}
//       <div
//         ref={sheetRef}
//         style={{
...
//           maxWidth: 500,
//           margin: "0 auto",
//           // resting height ~65% of screen
//           height: "65dvh",
//           // when keyboard opens, dvh shrinks → sheet shrinks → input stays pinned
//           maxHeight: "100dvh",
//           display: "flex",
//           flexDirection: "column",
//           background: "#111",
...
//         {/* ── Comment list ──
//          * flex:1 + minHeight:0 → expands to fill space, shrinks when keyboard opens.
//          * overflow-y:auto → scrollable.
//          */}
//         <div
//           ref={listRef}
...
//         {/* ── Input bar ──
//          * flexShrink:0 — never compressed by the list.
//          * Lives at the very bottom of the sheet.
//          * When keyboard opens → sheet's max-height shrinks via dvh →
//          * list compresses → input bar stays pinned at the new bottom edge.
//          * This is exactly the WhatsApp behavior: no JS, pure CSS.
//          * padding-bottom: env(safe-area-inset-bottom) handles notched phones
//          * but resolves to 0 when keyboard is open so no phantom gap appears.
//          */}
//         <div
//           style={{
//             flexShrink: 0,
...
//             {/*
//              * font-size:16px is critical on iOS — anything smaller triggers
//              * auto-zoom on focus. This is the #1 cause of jarring keyboard UX.
//              */}
//             <textarea
//               ref={inputRef}
//               value={text}
...
//                 outline: "none",
//                 resize: "none",
//                 fontSize: 16, // never go below 16px — prevents iOS zoom
//                 lineHeight: 1.4,
//                 fontFamily: "inherit",
//                 color: "rgba(255,255,255,0.85)",
//                 caretColor: "#60a5fa",
...
//                 borderRadius: "50%",
//                 border: "none",
//                 background: text.trim() ? "#60a5fa" : "rgba(255,255,255,0.07)",
...
function Empty() {
  return (
    ...
    >
      ...
      <p className="cbs-text-empty-h">No comments yet</p>
      <p className="cbs-text-empty-sub">Be the first to say something</p>
    </div>
  );
}
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <span className="cbs-text-title">Comments</span>
            <button
              onClick={dismiss}
              style={{
                width: 28,
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      <span className="cbs-text-username">
                        {c.user?.username}
                      </span>
                      {c.text}
                    </p>
                    <span className="cbs-text-timestamp">
                      {new Date(c.createdAt).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                      })}
...
const CSS = `
  ...
  /* 16px floor on textarea prevents iOS auto-zoom on focus — never lower this */
  .cbs-textarea {
    font-size: 16px;
  }

  /*
   * All other text uses clamp(min, fluid-vw, max):
   * - min: smallest readable size on a 320px phone
   * - preferred: scales with viewport width
   * - max: caps on tablets/desktop so text doesn't get oversized
   */
  .cbs-text-body      { font-size: clamp(12px, 3.2vw, 14px); }
  .cbs-text-username  { font-size: clamp(12px, 3.2vw, 14px); font-weight: 700; color: #fff; margin-right: 5px; }
  .cbs-text-timestamp { font-size: clamp(10px, 2.5vw, 12px); color: rgba(255,255,255,0.22); margin-top: 3px; display: block; }
  .cbs-text-title     { font-size: clamp(13px, 3.5vw, 15px); font-weight: 600; color: rgba(255,255,255,0.9); letter-spacing: 0.01em; }
  .cbs-text-delete    { font-size: clamp(12px, 3.2vw, 14px); font-weight: 500; }
  .cbs-text-empty-h   { font-size: clamp(12px, 3.2vw, 14px); font-weight: 600; color: rgba(255,255,255,0.25); margin: 0; }
  .cbs-text-empty-sub { font-size: clamp(11px, 2.8vw, 13px); color: rgba(255,255,255,0.13); margin: 0; }

  .cbs-textarea::placeholder { color: rgba(255,255,255,0.22); }
`;